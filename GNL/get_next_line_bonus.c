...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/04 00:55:11 by airis             #+#    #+#             */
/*   Updated: 2021/12/04 02:04:47 by airis            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*stat_line[FD_MAX] = {0};
	char		*out_line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	stat_line[fd] = ft_get_new_stat_line(stat_line[fd], fd);
	if (!stat_line[fd])
		return (NULL);
	out_line = ft_get_out_line(stat_line[fd]);
	...
	stat_line[fd] = ft_update_stat_line(stat_line[fd]);
	return (out_line);
}
